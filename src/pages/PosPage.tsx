...
const CameraScannerModal = lazy(() =>
  import("@/components/pos/CameraScannerModal").then((m) => ({
    default: m.CameraScannerModal,
  })),
);
...
export function PosPage() {
  ...
  const [ventaCompletada, setVentaCompletada] =
    useState<VentaCompletada | null>(null);
  const [ticketEntrega, setTicketEntrega] = useState<
    TicketEntregaInfo | undefined
  >();
  ...
  const [genericCosto, setGenericCosto] = useState("");
  ...
  function vaciarCarrito() {
    ...
    if (
      !window.confirm(
        "¿Vaciar el carrito? Se quitarán todos los productos agregados.",
      )
    )
      return;
    setCart([]);
  ...
  async function confirmarAgregarModal() {
    ...
    const validationError = await validateStockLotesParaVenta(
      producto,
      stockReq + stockUsado,
    );
    if (validationError) {
    ...
    if (stockUsado + stockReq > producto.stock) {
      setError(
        `Stock insuficiente. Quedan ${producto.stock} ${producto.unidad}`,
      );
      return;
    }
  ...
  async function addProductoDirecto(producto: Producto) {
    ...
    const existing = cart.find(
      (i) => i.producto_id === producto.id && i.modo_venta === "normal",
    );
  ...
  function addGeneric() {
    ...
    const costo = parseFloat(genericCosto);
    if (!genericNombre.trim() || isNaN(precio) || precio <= 0) {
      setError("Ingresa nombre y precio válido");
      return;
    }
    if (genericCosto.trim() === "" || isNaN(costo) || costo < 0) {
      setError("Ingresa el costo de compra (0 si no lo sabes — no contará en ganancia)");
      return;
    }
    const nombre = genericNombre.trim();
    setCart((prev) => [genericCartItem(nombre, precio, costo), ...prev]);
    ...
    setCart((prev) =>
      prev
        .map((item) => {
          ...
          const nuevaCant =
            Math.round(Math.max(0.001, item.cantidad + delta * step) * 1000) /
            1000;
          const preview = updateCartItemQuantity(item, nuevaCant);
          if (
            item.stock_disponible != null &&
            stockNecesario(preview) > item.stock_disponible
          ) {
      ...
      prev.map((item) =>
        item.key === key
          ? {
              ...
              descuento:
                Math.max(0, item.precio_original - precio) * item.cantidad,
            }
          : item,
      ),
    );
  ...
  const mixtaModal = productoModal
    ? permiteVentaUnidadSuelta(productoModal)
    : false;
        ...
        <div className="lg:col-span-2 rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-slate-400">
              ...
              <p className="text-sm">
                Escanea un producto o presiona Enter para buscar
              </p>
            </div>
          ) : (
              ...
              {cart.map((item) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      {item.es_generica && (
                        <>
                          {" · "}
                          Costo {formatMoney(item.costo_unitario)}
                          {item.costo_unitario > 0
                            ? ` · Gan. ${formatMoney(item.precio_unitario - item.costo_unitario)}/ud`
                            : " · sin costo en reporte"}
                        </>
                      )}
                    </p>
                  </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updatePrecio(item.key, parseFloat(e.target.value) || 0)
                    }
            ...
            {modalAgregar.modo === "unidad_suelta" &&
              productoModal.peso_estimado_unidad && (
                <p className="mb-4 text-xs text-slate-500">
                  Descuenta ~
                  {(
                    parseFloat(modalAgregar.cantidad || "0") *
                    productoModal.peso_estimado_unidad
                  ).toFixed(3)}{" "}
                  kg del stock
                </p>
      ...
      {showGeneric && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 sm:items-center">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mb-4 text-sm text-slate-500">
              Producto no registrado — nombre, precio de venta y costo de compra.
            </p>
            <div className="space-y-3">
              ...
              <input
                ...
                min="0"
                value={genericCosto}
                onChange={(e) => setGenericCosto(e.target.value)}
                placeholder="Costo compra (S/) *"
                className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-teal-500"
              />
              <p className="text-xs text-amber-800">
                El costo es obligatorio. Si pones 0, la venta cuenta en ingresos pero no en
                ganancia del reporte.
              </p>
              {genericPrecio && genericCosto && !isNaN(parseFloat(genericPrecio)) && !isNaN(parseFloat(genericCosto)) && (
                <p className="text-xs text-slate-600">
                  Ganancia estimada:{" "}
                  <strong>
                    {formatMoney(
                      Math.max(0, parseFloat(genericPrecio) - parseFloat(genericCosto)),
                    )}
                  </strong>{" "}
                  por unidad
                </p>
